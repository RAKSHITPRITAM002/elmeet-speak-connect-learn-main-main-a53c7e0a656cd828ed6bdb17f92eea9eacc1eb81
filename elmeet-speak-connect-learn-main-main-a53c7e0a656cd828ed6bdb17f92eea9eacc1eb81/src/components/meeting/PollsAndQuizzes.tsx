...
const PollsAndQuizzes: React.FC<PollsAndQuizzesProps> = ({ 
  ...
  const hasUserResponded = (pollId: string) => {
    ...
    return poll.responses.some(response => response.userId === user?.email);
  };
  ...
  return (
    <div className="polls-and-quizzes h-full flex flex-col">
            ...
            <DialogContent className="max-w-2xl">
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <RadioGroup 
                    value={newPoll.type} 
                    onValueChange={(value: PollType) => setNewPoll(prev => ({ ...prev, type: value }))}
      ...
      <Tabs 
        ...
        onValueChange={(value) => setActiveTab(value as "polls" | "results")}
      >
        ...
        <TabsContent value="polls" className="flex-1 p-4 overflow-auto">
          <ScrollArea className="h-full">
            {polls.filter(poll => poll.status !== "ended").length === 0 ? (
                          ...
                          <div className="space-y-4">
                            {poll.type === "single-choice" && (
                              <RadioGroup 
                                ...
                                onValueChange={(value) => handleSingleChoiceSelect(poll.id, value)}
                              >
                            ...
                            {poll.type === "open-ended" && (
                              <Textarea 
                                ...
                                value={userResponses[poll.id]?.[0] || ""}
                                onChange={(e) => handleOpenEndedResponse(poll.id, e.target.value)}
                              />
                            )}
                          </div>
                        ) : poll.status === "active" && hasUserResponded(poll.id) ? (
        ...
        <TabsContent value="results" className="flex-1 p-4 overflow-auto">
          <ScrollArea className="h-full">
            {polls.filter(poll => poll.status === "ended" || poll.responses.length > 0).length === 0 ? (
              <div className="text-center py-8 text-gray-500">
              ...
              <div className="space-y-4">
                {polls
                  .filter(poll => poll.status === "ended" || poll.responses.length > 0)
                  .map(poll => (
                    <Card key={poll.id}>
                      <CardHeader>
                        <div className="flex justify-between items-start">
